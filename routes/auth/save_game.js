...
router.put('/', async (req, res) => {
    const {username, x, y, mapId, aoId, quanId} = req.body;

    if (!username) return res.status(400).send('Ten nguoi dung khong duoc de trong');

    try {
        const [results] = await pool.query('UPDATE users SET x = ?, y = ?, mapId = ?, aoId = ?, quanId = ? WHERE username = ?', [x, y, mapId, aoId, quanId, username]);
